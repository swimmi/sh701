...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="biao">
  <tr>
    <td height="30" align="center" background="../images/contop_bg.jpg">注册单列表</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
...
SQL="select * from view_xiaofei_hy1 where xiaofei_qr_flag=0  and xiaofei_flag_10=0 and xiaofei_zuofei_flag=0 and zhuce_flag=1 and xiaofei_date between '"&session("date1")&"' and '"&session("date2")&"' "&search_sql&" order by xiaofei_date "
  ...
  <tr>
    <td><table width="100%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="999999">
      <tr bgcolor="#E3E3E3">
        ...
        <td height="35" align="center" bgcolor="#CCCCCC" class="title">商品价格</td>
        <td height="35" align="center" bgcolor="#CCCCCC" class="title">购买数量</td>
        <td height="35" align="center" bgcolor="#CCCCCC" class="title">消费价格</td>
        <td height="35" align="center" bgcolor="#CCCCCC" class="title">提交时间</td>
        <td align="center" bgcolor="#CCCCCC" class="title">订单状态</td>
        <td height="35" align="center" nowrap="nowrap" bgcolor="#CCCCCC" class="title">操作方法</td>
      </tr>
      <%
		  	dim i
			i=1
...
%>
  <tr bgcolor="FFFFFF" onMouseOver="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
        ...
        <td height="35" align="center" class="content"><%=rs("xiaofei_money1")%></td>
        <td height="35" align="center" class="content"><%=rs("xiaofei_pro_count")%></td>
        <td height="35" align="center" class="content"><%=rs("xiaofei_money1")%>元</td>
        <td height="35" align="center" class="content"><%=formatdatetime(cstr(rs("xiaofei_date")),2)%></td>
        <td align="center" class="content"><%if rs("fahuo_flag")=0 then%>
          未发货
          <%end if%>
  ...
  <%end if%></td>
        ...
        <%if rs("fahuo_flag")<>3 then%>
		<a href="tui.aspx?id=<%=rs("xiaofei_id")%>" onClick="javascript:if(!confirm('\n特别提示！？\n\n请慎重进行该操作。')) return false; ">退货</a>&nbsp;
        <%else%>
        <a href="tui.aspx?id=<%=rs("xiaofei_id")%>">退货确认</a>
        <%end if%>
        <%if rs("fahuo_flag")<>3 then%>&nbsp;<a href="fahuo.aspx?id=<%=rs("xiaofei_id")%>">物流发货</a><%end if%>        </td>
      </tr>
      <%
			  i=i+1
			  icolor=icolor+1
