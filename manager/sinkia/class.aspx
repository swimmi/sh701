...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
  <tr>
    <td height="80" colspan="2">类别管理</td>
  </tr>
  <tr>
    <td width="2%" align="center" bgcolor="EEEEEE"><img src="../images/3.jpg" width="13" height="10" /></td>
    <td width="98%" height="40" bgcolor="EEEEEE" class="titlea">首页 &gt; 商品管理 &gt; 类别管理</td>
  </tr>
</table>
<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="FFB849">
  <tr class="biao">
    <td height="40" style="padding-left:10px;"><img src="../images/4.jpg" width="18" height="15" />类别管理</td>
  </tr>
  <tr>
...
%>
  <tr bgcolor="FFFFFF" onmouseover="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
    ...
    <td width="300"  align="center" class="content">
		<a href="?action=mod&id=<%=Rs("id")%>">修改</a> 	
		<a href="?id=<%=Rs("id")%>&action=del" onClick="if(confirm('你确定你要这样操作吗？删除将不能恢复！并且删除此类别下的商品~！请慎重~！')){return true;}else{return false;}">删除</a>
	</td>
  </tr>
 <%
				  set rsm = conn.execute("select * from tbl_sinkia_class where pid="&rs("id"))
				  while not rsm.eof
       		 %> 
  <tr bgcolor="#f8f8f8" onmouseover="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
    ...
    <td width="300"  align="center" class="content">
		...
		<a href="?action=mod&id=<%=rsm("id")%>">修改</a> <a href="?id=<%=rsm("id")%>&action=del" onClick="if(confirm('你确定你要这样操作吗？删除将不能恢复！并且删除此类别下的商品~！请慎重~！')){return true;}else{return false;}">删除</a></td>
  </tr>
             <%
			    rsm.movenext
			    wend
...
<%end sub
sub addsave()
response.write "<br><table width='96%' border='0' align='center' cellpadding='3' cellspacing='1' bgcolor='#C6C3C6'><tr bgcolor='#ffffff'><td height=120 valign=middle ><div align=center  style='font-size:12px;'>添加成功　　<a href=?action=add><font color='#FF0000' style='font-size:12px;'>继续添加</a>　　<a href=?><font color='#FF0000' style='font-size:12px;'>返回列表</font></a></div></td></tr><meta http-equiv='refresh' content='2 url=?'></table>"
end sub
sub del()

SQL="select * from tbl_sinkia_class where pid="&request("id")&""
set rs=server.CreateObject("adodb.recordset")
...
SQL="select * from tbl_product where pro_pid="&request("id")&""
set rs=server.CreateObject("adodb.recordset")
rs.Open SQL,conn,1,1
if not rs.eof and not rs.bof then
  	response.Write "<script language=javascript>alert('请先删除该分类的商品!');history.go(-1);</script>"
	response.end
end if
rs.close
set rs=nothing
...
sql="delete from tbl_sinkia_class where id="&request("id")
conn.execute sql

sql="delete from tbl_product where pro_pid="&request("id")
conn.execute sql

sql="delete from tbl_sinkia_news where pid="&request("id")
conn.execute sql



response.write "<br><table width='96%' border='0' align='center' cellpadding='3' cellspacing='1' bgcolor='#C6C3C6'><tr bgcolor='#ffffff'><td height=120 valign=middle ><div align=center  style='font-size:12px;'>删除成功　　<a href=?><font color='#FF0000' style='font-size:12px;'>返回列表</font></a></div></td></tr><meta http-equiv='refresh' content='1 url=?'></table>"

 end sub
 sub adds()
 			Set Rsc = Conn.Execute("select * from tbl_sinkia_class where pro_class='"&request.Form("Name")&"'")
				
			'IF not Rsc.Eof Then
			'			Response.Write("<script>alert('类别已经存在');javascript:history.go(-1);<'/script>")
			'Else
					Rs.Open ("select * from tbl_sinkia_class"),Conn,3,3
							Rs.Addnew
							Rs("pro_class")  = request.Form("Name")
						...
						Response.Write "<br><table width='96%' border='0' align='center' cellpadding='3' cellspacing='1' bgcolor='#C6C3C6'><tr bgcolor='#ffffff'><td height=120 valign=middle ><div align=center  style='font-size:12px;'>添加中请稍候......　　</table>"
 ...
 sub mods()
  sql="select * from tbl_sinkia_class where  id="&request("id")
	rs.open sql,conn,1,1
 %>
 <br><table width="96%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#c1c1c1" class="content">
   <form action="?action=modsaves" name="form1" method="post">
	  <tr>
       ...
       <td width="50%" height="40" align="left" bgcolor="#FFFFFF"><input name="Name" type="text" size="40" value="<%=Rs("pro_class")%>">
         * </td>
     </tr>
	...
	<td height="40" align="left" bgcolor="#FFFFFF">
     <input name="radio" type="radio"size="40" <%if rs("type")=1 then%>checked<%end if%> value="1"> 显示
	 <input name="radio" type="radio"size="40"  value="0">不显示
	</td>
    </tr>
...
sub modsaves()
			...
			Set Rsc = Conn.Execute("select * from tbl_sinkia_class where pro_class='"&request.Form("Name")&"' and id <> "&request("ids"))
				
			IF not Rsc.Eof Then
						Response.Write("<script>alert('类别已经存在');javascript:history.go(-1);</script>")
			Else
					Rs.Open ("select * from tbl_sinkia_class where id="&request("ids")),Conn,3,3
						Rs("pro_class")  =request.Form("Name")						
					...
					Response.Write "<br><table width='96%' border='0' align='center' cellpadding='3' cellspacing='1' bgcolor='#C6C3C6'><tr bgcolor='#ffffff'><td height=120 valign=middle ><div align=center style='font-size:12px;'>修改中请稍候......　　</table>"
					Response.Write "<meta http-equiv='refresh' content='1 url=?action=modsave'>"
...
sub modsave()
response.write "<br><table width='96%' border='0' align='center' cellpadding='3' cellspacing='1' bgcolor='#C6C3C6'><tr bgcolor='#ffffff'><td height=120 valign=middle ><div align=center style='font-size:12px;'>修改成功　　</div></td></tr><meta http-equiv='refresh' content='1 url=?'></table>"
end sub%></td>
              </tr>
</table>
