...
<%
	if request("action")="add" then
		...
		if request("leibie")="" then
			response.Write "<script language=javascript>alert('请填写类别！');location.href='class4.aspx';</script>"
			response.end
		end if
		...
		response.Write "<script language=javascript>alert('添加成功！');location.href='class14.aspx';</script>"
		response.end
...
<%
	...
	if request("Submit")="修改" then
		...
		conn.execute("update leibie set leibie='"&request("leibie")&"' where id='"&request("id")&"'")
	
		response.Write "<script language=javascript>alert('操作成功！');location.href='class14.aspx';</script>"
		response.end
	...
	if request("action")="del" then
		...
		conn.execute("delete leibie where id='"&request("id")&"'")
    ...
    <tr>
      <td align="center" bgcolor="#FFFFFF"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0" class="content">
        <tr>
          ...
          <td width="110" height="40" align="left" ><input type="submit" name="Submit" value="修改类别" style="border:1px #CCCCCC solid; background-color:#FFFFFF; line-height:20px; font-family:'微软雅黑';" onClick="javascript:location.href='class14.aspx'" disabled/></td>
	  ...
	  <form id="form1" name="form1" method="post" action="?">
        <table width="99%" border="0" cellpadding="0" cellspacing="0" style="border:1px #CCCCCC solid">
          <tr>
            <td width="100" height="30" align="left">&nbsp;&nbsp;&nbsp;&nbsp;类别：</td>
	   ...
	   <hr color="#FF9900" />
        ...
        <tr bgcolor="#e1e1e1" class="titlea1">
          <td width="50%" height="40" align="center" >类别</td>
          </tr>
<%
...
<form id="form1" name="form1" method="post" action="?">
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td><input type="text" name="leibie" value="<%=rs("leibie")%>" style="width:250px; height:20px; line-height:20px; font-family:'微软雅黑'" />
