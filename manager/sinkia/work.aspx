...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
  <tr>
    <td height="80" colspan="2">人才招聘</td>
  </tr>
  <tr>
    <td width="2%" align="center" bgcolor="EEEEEE"><img src="../images/3.jpg" width="13" height="10" /></td>
    <td width="98%" height="40" bgcolor="EEEEEE" class="titlea">首页 &gt; 商城信息 &gt; 人才招聘</td>
  </tr>
</table>
<table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="FFB849">
  <tr class="biao">
    <td height="40" style="padding-left:10px;"><img src="../images/4.jpg" width="18" height="15" />人才招聘</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	Rs.Open ("select * from tbl_work where lei=0"),Conn,1,1
...
%>
  <tr bgcolor="FFFFFF" onmouseover="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
    ...
    <td width="300"  align="center" class="content">
		<a href="webcomd.aspx?id=<%=rs("id")%>">修改内容</a> <a href="?action=mod&id=<%=Rs("id")%>">修改</a> <a href="?id=<%=Rs("id")%>&action=del" onClick="if(confirm('你确定你要这样操作吗？删除将不能恢复！请慎重！')){return true;}else{return false;}">删除</a>	</td>
  </tr>
 <%
				  set rsm = conn.execute("select * from tbl_work where lei="&rs("id"))
				  while not rsm.eof
       		 %> 
  <tr bgcolor="FFFFFF" onmouseover="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
  	<td height="22" align="left" class="content" style="padding-left:5px;">　　|--<%=rsm("title")%></td>
    <td width="300"  align="center" class="content"><a href="?action=mod&id=<%=rsm("id")%>">修改</a> <a href="?id=<%=rsm("id")%>&action=del" onClick="if(confirm('你确定你要这样操作吗？删除将不能恢复！并且删除此类别下的商品~！请慎重~！')){return true;}else{return false;}">删除</a></td>
  </tr>
             <%
			    rsm.movenext
			    wend
...
SQL="select * from tbl_work where lei="&request("id")&""
set rs=server.CreateObject("adodb.recordset")
...
sql="delete from tbl_work where id="&request("id")
conn.execute sql
...
response.write "<br><table width='96%' border='0' align='center' cellpadding='3' cellspacing='1' bgcolor='#C6C3C6'><tr bgcolor='#ffffff'><td height=120 valign=middle ><div align=center>删除成功　　<a href=?><font color='#FF0000'>返回列表</font></a></div></td></tr><meta http-equiv='refresh' content='2 url=?'></table>"
 ...
 sub adds()
 			Set Rsc = Conn.Execute("select * from tbl_work where title='"&request.Form("Name")&"'")
				
			IF not Rsc.Eof Then
						Response.Write("<script>alert('类别已经存在');javascript:history.go(-1);</script>")
			Else
					Rs.Open ("select * from tbl_work"),Conn,3,3
							Rs.Addnew
 ...
 sub mods()
  sql="select * from tbl_work where  id="&request("id")
	rs.open sql,conn,1,1
 %>
...
sub modsaves()
			...
			Set Rsc = Conn.Execute("select * from tbl_work where title='"&request.Form("Name")&"' and id <> "&request("ids"))
				
			IF not Rsc.Eof Then
						Response.Write("<script>alert('类别已经存在');javascript:history.go(-1);</script>")
			Else
					Rs.Open ("select * from tbl_work where id="&request("ids")),Conn,3,3
						Rs("title")  =request.Form("Name")	
						Rs.Update
						Rs.Close
					set rs=nothing
