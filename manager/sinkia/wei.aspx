...
-->
				...
				<td width="90" align="right"><input type="submit" name="Submit2" value="生成二维码" style="border:1px #CCCCCC solid; background-color:#FFFFFF; line-height:15px; font-family:'微软雅黑';" onClick="if(confirm('你确定生成二维码吗?')){return true;}else{return false;}"/></td>
				</tr>
			</table>
		    </form>
...
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td height="40" align="right"><input type="submit" name="Submit22" value="批量导出二维码" style="border:1px #CCCCCC solid; background-color:#FFFFFF; line-height:20px; font-family:'微软雅黑';" onClick="javascript:location.href='weidao.aspx?tushu=<%=request("tushu")%>&banci=<%=request("banci")%>&yingci=<%=request("yingci")%>&id=<%=request("id")%>'"/>
      <input type="submit" name="Submit23" value="全部删除" style="border:1px #CCCCCC solid; background-color:#FFCC00; line-height:20px; font-family:'微软雅黑';" onClick="javascript:if(!confirm('\n特别提示!删除后将无法进行恢复。'))  return  false; location.href='weidel.aspx?tushu=<%=request("tushu")%>&banci=<%=request("banci")%>&yingci=<%=request("yingci")%>&id=<%=request("id")%>'"/></td>
  </tr>
</table>
