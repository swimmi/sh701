...
if request("Submit2")="添加" then
	...
	Response.Write "<script language=javascript>alert('操作成功！');location.href='address2.aspx?shen_id="&request("shen_id")&"';</script>"
	Response.End 
end if

if request("Submit")="修改" then
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from tbl_address3 where id="&request("id")&""
	rs.open sql,conn,1,3
...
if request("action")="del" then
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from tbl_address4 where address="&request("id")&""
	rs.open sql,conn,1,3
	if not rs.eof and not rs.bof then
		Response.Write "<script language=javascript>alert('操作失败，请先删除镇！');location.href='address2.aspx?shen_id="&request("shen_id")&"';</script>"
		Response.End 
	end if
	rs.close
	set rs=nothing

	conn.execute("delete from tbl_address3 where id='"&request("id")&"'")
...
<form action="address2.aspx" method="post">
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="content">
  <tr>
    <td height="40" align="center">添加区名：
...
<form action="address2.aspx" method="post">
  <tr bgcolor="FFFFFF" onMouseOver="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
    ...
    <a href="address2.aspx?id=<%=rs("id")%>&action=del&shen_id=<%=request("shen_id")%>" onClick="javascript:if(!confirm('\n特别提示！\n\n请慎重进行该操作！\n删除后将无法进行恢复。'))  return  false;">删除</a>
	 <!--<a href="address4.aspx?shen_id=<%=rs("id")%>">添加镇名</a>-->
	 </td>
  </tr>
</form>
  <%rs.movenext
