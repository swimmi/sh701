...
if request("Submit2")="添加" then
	...
	Response.Write "<script language=javascript>alert('操作成功！');location.href='address4.aspx?shen_id="&request("shen_id")&"';</script>"
	Response.End 
end if

if request("Submit")="修改" then
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from tbl_address4 where id="&request("id")&""
	rs.open sql,conn,1,3
	rs("dizhi")=request("dizhi2")
	rs("youbian")=request("youbian2")
	rs.update
	rs.close
	set rs=nothing
...
if request("action")="del" then
	...
	if not rs.eof and not rs.bof then
		Response.Write "<script language=javascript>alert('操作失败！');location.href='address4.aspx?shen_id="&request("shen_id")&"';</script>"
		Response.End 
	end if
	rs.close
	set rs=nothing

	conn.execute("delete from tbl_address4 where id='"&request("id")&"'")
...
<form action="address4.aspx" method="post">
...
<%
SQL="select * from tbl_address4 where address="&request("shen_id")&""
set rs=server.CreateObject("adodb.recordset")
...
<form action="address4.aspx" method="post">
  <tr bgcolor="FFFFFF" onMouseOver="this.style.backgroundColor='#F1F1F1'"  onMouseOut="this.style.backgroundColor=''">
    ...
    <a href="address4.aspx?id=<%=rs("id")%>&action=del&shen_id=<%=request("shen_id")%>" onClick="javascript:if(!confirm('\n特别提示！\n\n请慎重进行该操作！\n删除后将无法进行恢复。'))  return  false;">删除</a></td>
  </tr>
</form>
  <%rs.movenext
		  loop%>
