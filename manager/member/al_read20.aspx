...
if Request("search")<>"" then
	select case Request("type")
		case 1
			search_sql=" and sh_name='"&Request("search")&"'"
		case 2
...
SQL="select * from view_huiyuan where   huiyuan_id<>'A00000000' and huiyuan_jiebie=2 and huiyuan_suoding=0 and rz=2  "&search_sql&"   and huiyuan_add_time between '"&session("date1")&"' and '"&session("date2")&"' order by huiyuan_add_time desc"
...
%>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="title">
    <tr>
      <td height="80" colspan="2">商户列表</td>
    </tr>
    <tr>
      <td width="2%" align="center" bgcolor="EEEEEE"><img src="../images/3.jpg" width="13" height="10" /></td>
      <td width="98%" height="40" bgcolor="EEEEEE" class="titlea">首页 &gt; 商户管理 &gt; 商户列表</td>
    </tr>
  </table>
  <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="FFB849">
    <tr class="biao">
      <td height="40" style="padding-left:10px;"><img src="../images/4.jpg" width="18" height="15" />商户列表</td>
    </tr>
    <tr>
...
%>
          <tr bgcolor="FFFFFF" onMouseOver="this.style.backgroundColor='#F1F1F1'"  onmouseout="this.style.backgroundColor=''">
            <td align="center" class="content"  ><%=rs("sh_name")%></td>
            <td align="center" class="content"  ><%=rs("huiyuan_name")%></td>
            <td height="30" align="center" class="content"><%=rs("huiyuan_add_time")%></td>
          <td align="center" class="content">
            <%if rs("rz1")=0 then%>未设置<%end if%>
            <%if rs("rz1")=1 then%>已设置<%end if%>
            <%if rs("rz1")=2 then%>设置中<%end if%>
            <%if rs("rz1")=3 then%>商户设置<%end if%>
            <%if rs("rz1")=4 then%>设置成功<%end if%>		  </td>
          <td height="30" align="center" class="content"><%if rs("huiyuan_suoding")=0 then%>
            <a href="dj.aspx?flag=0&amp;id=<%=rs("huiyuan_id")%>">正常</a>
            <%else%>
            <a href="dj.aspx?flag=1&amp;id=<%=rs("huiyuan_id")%>"><font color="#FF0000">查封中</font></a>
          <%end if%></td>
          <td height="30" align="center" class="content">
		  	<%if rs("sh_flag")=0 then%>
			  <a href="al_infod1.aspx?huiyuan_id=<%=rs("huiyuan_id")%>&flag=1">设为首页</a>
			 <%else%>
			  <a href="al_infod1.aspx?huiyuan_id=<%=rs("huiyuan_id")%>&flag=0">取消设为首页</a>
			 <%end if%>
			  &nbsp;
			  <a href="al_infod.aspx?huiyuan_id=<%=rs("huiyuan_id")%>">修改资料</a>&nbsp;
			  <a href="rz.aspx?huiyuan_id=<%=rs("huiyuan_id")%>">安全认证</a>&nbsp;
			  <%if rs("rz1")=2 then%><a href="yh.aspx?huiyuan_id=<%=rs("huiyuan_id")%>">银行认证</a><%end if%>		  </td>
        </tr>
          <%
			  i=i+1
			  icolor=icolor+1
