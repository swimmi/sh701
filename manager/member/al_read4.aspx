...
SQL="select * from view_huiyuan where   huiyuan_id<>'A00000000' and huiyuan_jiebie=2 and huiyuan_suoding=0  "&search_sql&"   and huiyuan_add_time between '"&session("date1")&"' and '"&session("date2")&"' order by huiyuan_add_time desc"
...
%>
          ...
          <td height="30" align="center" class="content"><%if rs("huiyuan_suoding")=0 then%>
            ...
            <%else%>
            <a href="dja.aspx?flag=1&amp;id=<%=rs("huiyuan_id")%>"><font color="#FF0000">查封中</font></a>
          <%end if%></td>
          <td height="30" align="center" class="content"><a href="al_info_update.aspx?huiyuan_id=<%=rs("huiyuan_id")%>">修改资料</a></td>
        </tr>
          <%
			  i=i+1
			  icolor=icolor+1
